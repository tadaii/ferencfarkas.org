<script>
	...
	async function loadData() {
		const responses = {}
		await Promise.all(Object.entries(endpoints).map(([ key, url ]) => fetch(url)
			.then(res => res.json())
	...
	function handleFacets({ activeFacets, works }) {
		return activeFacets.length
			? works
				.filter(work => activeFacets
					.every(facet => work.facets.includes(facet)))
			: works
	}
					...
					<ul class="works--list">
						{#each renderedResults as work, index (work.id)}
							<Work
								...
								on:showReworks={e => state.reworksOf = state.reworksOf === e.detail ? '' : e.detail}
							/>
						{/each}
					</ul>
				</div>
